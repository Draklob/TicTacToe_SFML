...
void GameState::Init()
{
	...
	// Loading the textures for GameState
	...
	// Placing the sprites to desired positions
	...
	// Grid empty when the game starts.
	for( int x = 0; x < 3; x++ )
	{
		for( int y = 0; y < 3; y++ )
...
void GameState::CheckAndPlacePiece()
{
	...
	sf::Vector2f gapOutsideGrid = sf::Vector2f( (SCREEN_WIDTH - gridSize.width) / 2, (SCREEN_HEIGHT - gridSize.height) / 2);
	

	sf::Vector2f gridLocalTouchPos = sf::Vector2f( touchPoint.x - gapOutsideGrid.x, touchPoint.y - gapOutsideGrid.y);
	...
	if( _gridArray[column -1][row - 1] == EMPTY_PIECE)
	{
		...
		_gridPieces[column -1 ][ row -1 ].setColor( sf::Color::White);
	}
